---
featuredOn:
  - sea_level_change 
id: "introduction_sea_level_change"
name: Introduction to Sea Level Change
description: "
  Global sea levels are changing at rates that are unprecedented over the past 2,500 years.
  Explore how sea level change has increased over time and learn how Earth data are helping communities plan for changing sea levels.
"
asLink:
  url: https://sealevel.nasa.gov
media:
  src: ::file ./external_headers/connected_earth_sea_level_rise.png
  alt: "
    Glacier front with jagged blue ice meeting a calm, reflective body of water.
    Pieces of ice float in the water, mirroring the glacier's edge.
    Rocky, brown mountain peaks rise behind the ice, contrasting the icy blue with earthy tones under a cloudy sky.
  "
  author:
    name: "NASA"
    url: "https://nasa.gov"
pubDate: 2023-11-28
taxonomy:
  - name: Topics
    values:
      - sea level change
---
